...
//		}
		...
		for (int right = 0, left = 0; right < s.length(); right++) {
			int indexOfFirstAppearanceOnTheSubstring = s.indexOf(s.charAt(right), left);
			if(indexOfFirstAppearanceOnTheSubstring != right) {
				left = indexOfFirstAppearanceOnTheSubstring+1;
			}
			maxLength = Math.max(maxLength, right-left+1);
		}
